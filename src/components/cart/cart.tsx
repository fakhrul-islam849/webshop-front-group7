...
export default function Cart() {
  ...
  if (cartItem?.length > 0) {
    cartItem.map((item: any) => {
      totalPrice = totalPrice + Number(item.unit_price) * Number(item.quantity);
    });
  }
  return (
    <div className="flex flex-col justify-between w-full h-full">
      <div className="relative flex items-center justify-between w-full border-b ltr:pl-5 rtl:pr-5 md:ltr:pl-7 md:rtl:pr-7 border-border-base">
        <Heading variant="titleMedium">Price Calculator</Heading>
        ...
        <div className="flex pb-5 md:pb-7">
          <div className="ltr:pr-3 rtl:pl-3">
            ...
            <Text className="leading-6">
              Not available for purchase. You may calculate the desired cost.
            </Text>
          </div>
