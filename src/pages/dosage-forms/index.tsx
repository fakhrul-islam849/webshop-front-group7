...
import { useGetDosagesTypeQuery } from '../../features/dosages/dosagesApi';
...
export default function Allopathic() {
    ...
    const { data, isLoading, isSuccess, isError } = useGetDosagesTypeQuery({
        page: pageNumber,
    ...
    if (!isLoading && !isError && data?.data?.length > 0) {
        content = data?.data.map((item: any) => (
            <DosagesTypeCard key={item.id} item={item} />
        ));
    }
