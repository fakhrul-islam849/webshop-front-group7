...
import { useGetPharmaceuticalListQuery } from '../../features/pharmaceutical/pharmaceuticalApi';
...
import PharmaceuticalCard from '@components/card/pharmaceutical-card';
...
export default function Allopathic() {
  ...
  const { data, isLoading, isSuccess, isError } = useGetPharmaceuticalListQuery(
    {
      page: pageNumber,
  ...
  if (!isLoading && !isError && data?.data?.length > 0) {
    content = data?.data.map((item: any) => (
      <PharmaceuticalCard key={item.id} item={item} />
    ));
  }
